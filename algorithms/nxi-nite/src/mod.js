(() => {
  ...
  // algorithms/nxi-nite/src/preset.ts
  var spec = {
    id: "nxi-nite",
    knobs: [
      "scheme",
  ...
  // algorithms/nxi-nite/src/passes.ts
  var DEFAULT_HOUR = 12;
  var FLOOR = 4.5 + 0.2;
  var WARM_HUE = 65;
  var COOL_HUE = 250;
  var DIM_AMPLITUDE = 0.12;
  var WARMTH_AMPLITUDE = 18;
  function hourOf(knobs) {
  ...
  var DIM_LADDER = ["--bg-sunken", "--body-bg", "--bg-0", "--bg-1", "--bg-2", "--bg-3"];
  var DIM_EXTRA = ["--surface-overlay", "--field-bg"];
  var SURFACE_TOKENS = [...DIM_LADDER, ...DIM_EXTRA];
  var WARMTH_TOKENS = SURFACE_TOKENS;
  ...
  function shiftHue(hue3, target, degrees) {
    let d = ((target - hue3) % 360 + 360) % 360;
    if (d > 180) d -= 360;
    const move = Math.sign(d) * Math.min(Math.abs(d), degrees);
    return ((hue3 + move) % 360 + 360) % 360;
  }
  function relLuminance(value) {
    return contrast2(value, "#000000") * 0.05 - 0.05;
  }
  function achievableFloor2(declaredFloor, bgValue) {
    const ceiling = Math.max(contrast2("#000000", bgValue), contrast2("#ffffff", bgValue));
    return Math.min(declaredFloor, ceiling - 0.05);
  }
  function matchLuminance(color, targetL) {
    const lum = (l) => relLuminance(formatCss2(withLightness(color, l)));
    let lo = 0;
    let hi = 1;
    for (let i = 0; i < 24; i++) {
      const mid = (lo + hi) / 2;
  ...
  function warmthRun(register, ctx) {
    const n = phaseOf(ctx);
    ...
    for (const name of WARMTH_TOKENS) {
      ...
      if (ctx.pinned[name] !== void 0) continue;
      ...
      const tinted = oklch2(color.l, newChroma, newHue, color.alpha);
      out[name] = formatCss2(matchLuminance(tinted, targetLum));
    }
    return out;
  }
  function dimRun(register, ctx) {
    const n = phaseOf(ctx);
    const desired = Math.max(0, n) * DIM_AMPLITUDE;
    if (desired === 0) return { ...register };
    const ladderPinned = DIM_LADDER.some((name) => ctx.pinned[name] !== void 0);
    const pole = ctx.scheme === "dark" ? 0 : 1;
    const targets = ladderPinned ? DIM_EXTRA : SURFACE_TOKENS;
    const dimmed = (value, t2) => {
      const color = asColor(value);
      if (!color) return null;
      return formatCss2(withLightness(color, color.l + (pole - color.l) * t2));
    };
    const inks = RESTORE_PAIRS.filter((p4) => ctx.pinned[p4.fg] === void 0).map((p4) => ({ value: register[p4.fg], floor: p4.floor })).filter((x) => x.value !== void 0 && !hasAlpha(x.value));
    const textSurfaces = targets.filter(
      (s) => s === "--bg-0" || s === "--bg-1" || s === "--bg-2"
    );
    const bg0Value = register["--bg-0"];
    const bg0Dims = !ladderPinned && bg0Value !== void 0 && !hasAlpha(bg0Value);
    const fills = bg0Dims ? FILL_TOKENS.filter((name) => ctx.pinned[name] === void 0).map((name) => register[name]).filter((v) => v !== void 0 && !hasAlpha(v)) : [];
    const inkSafe = (t2) => textSurfaces.every((surface) => {
      const sv = register[surface];
      if (sv === void 0 || hasAlpha(sv)) return true;
      const sdim = dimmed(sv, t2);
      if (sdim === null) return true;
      return inks.every(
        ({ value, floor }) => contrast2(value, sdim) >= Math.min(floor, contrast2(value, sv)) - 5e-3
      );
    });
    const fillSafe = (t2) => {
      if (!bg0Dims || bg0Value === void 0) return true;
      const sdim = dimmed(bg0Value, t2);
      if (sdim === null) return true;
      return fills.every(
        (f3) => contrast2(f3, sdim) >= Math.min(SURFACE_SEPARATION, contrast2(f3, bg0Value)) - 5e-3
      );
    };
    const safe = (t2) => inkSafe(t2) && fillSafe(t2);
    let t = desired;
    if (!safe(t)) {
      let lo = 0;
      let hi = desired;
    ...
    for (const name of targets) {
      ...
      const value = register[name];
      if (value === void 0 || hasAlpha(value)) continue;
  ...
  var FILL_TOKENS = ["--accent", "--neutral", "--success", "--warn", "--danger", "--info"];
  function clearBounded(fgValue, bgValue, bg0Value, scheme, floor) {
    const reachable = achievableFloor2(floor, bgValue);
    if (contrast2(fgValue, bgValue) >= reachable - 0.01) return fgValue;
    const fg = asColor(fgValue);
    const bg = asColor(bgValue);
    if (!fg || !bg) return fgValue;
    const repaired = formatCss2(enforceContrastFloor(fg, bg, scheme, reachable));
    if (contrast2(repaired, bg0Value) < contrast2(fgValue, bg0Value) - 0.01) return fgValue;
    return repaired;
  }
  function contrastRestoreRun(register, ctx) {
    const out = { ...register };
    const bg0Value = out["--bg-0"];
    if (bg0Value === void 0) return out;
    for (const { fg, bg, floor } of RESTORE_PAIRS) {
      if (ctx.pinned[fg] !== void 0) continue;
      const fgValue = out[fg];
      const bgValue = out[bg];
      if (fgValue === void 0 || bgValue === void 0 || hasAlpha(fgValue)) continue;
      out[fg] = clearBounded(fgValue, bgValue, bg0Value, ctx.scheme, floor);
    }
    for (const { border, surface, floor } of BORDER_RESTORE) {
      if (ctx.pinned[border] !== void 0) continue;
      const borderValue = out[border];
      const surfaceValue = out[surface];
      if (borderValue === void 0 || surfaceValue === void 0 || hasAlpha(borderValue)) continue;
      const seed = asColor(borderValue);
      const surf = asColor(surfaceValue);
      if (!seed || !surf) continue;
      out[border] = formatCss2(borderForContrast(seed, surf, floor));
    }
    return out;
  }
  function warmthPass() {
  ...
  function contrastRestorePass() {
    return { name: "contrast-restore", run: contrastRestoreRun };
  }
  function nxiNitePasses(preset, opts) {
