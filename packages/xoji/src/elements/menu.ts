...
export class XojiMenu extends XojiElement {
	protected override get styleMode(): StyleMode {
		return "auto";
	}

	private uid = `xoji-menu-${menuSeq++}`;
	...
	private get trigger(): HTMLButtonElement | null {
		return this.root.querySelector(".xoji-menu__trigger");
	}

	private get popup(): HTMLElement | null {
		return this.root.querySelector(".xoji-menu__popup");
	}
	...
	private warnIfUnlabeled(): void {
		if (!this.getAttribute("label")) {
			console.warn("xoji-menu: no trigger label. Provide a `label` so the menu button is named.");
		}
	}
