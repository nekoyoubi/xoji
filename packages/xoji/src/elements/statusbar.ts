...
export class XojiStatusbar extends XojiElement {
	...
	private elementId = `xoji-statusbar-${Math.random().toString(36).slice(2, 8)}`;
	...
	private markRuleStarts(): void {
		...
		for (const el of this.slotElements()) {
			if (el.classList.contains("xoji-statusbar__spacer")) {
				runStart = true;
				continue;
			}
	...
	private get menu(): HTMLElement | null {
		return this.root.querySelector(".xoji-statusbar__overflow");
	}

	private get overflowPopover(): HTMLElement | null {
		return this.root.querySelector(".xoji-statusbar__overflow-popover");
	}

	private get bar(): HTMLElement | null {
		return this.root.querySelector(".xoji-statusbar");
	}
	...
	private cellsFrom(elements: HTMLElement[]): Cell[] {
		return elements
			.filter((el) => !el.classList.contains("xoji-statusbar__spacer"))
			.map((el, naturalIndex) => {
	...
	private restoreAll(): void {
		for (const el of this.hiddenEls) {
			...
			el.removeAttribute("data-xoji-collapsed");
		}
	...
	private setMenuLabel(count: number): void {
		...
		if (!menu) return;
		const trigger = menu.querySelector(".xoji-statusbar__overflow-trigger");
	...
	private fillPopover(dropped: Cell[]): void {
		...
		for (const cell of dropped) {
			...
			clone.removeAttribute("data-xoji-collapsed");
			clone.classList.add("xoji-statusbar__overflow-item");
	...
	private recompute = (): void => {
		...
		if (cells.length > 0 && !this.fits(bar)) {
			...
			for (const cell of dropOrder) {
				...
				cell.el.setAttribute("data-xoji-collapsed", "");
	...
	/** The fragment paints `.xoji-statusbar` synchronously once its fill is warm, but the very
	...
	private positionPopover = (event: Event): void => {
		const popover = this.overflowPopover;
		const trigger = this.menu?.querySelector(".xoji-statusbar__overflow-trigger") as HTMLElement | null;
...
define("xoji-statusbar", XojiStatusbar);
