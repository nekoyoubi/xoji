...
export class XojiLink extends XojiElement {
	...
	private warnIfHrefless(): void {
		if (this.href === null) {
			console.warn(
				"xoji-link: no `href` provided. Rendering inert text — set an `href` so it behaves and announces as a link.",
			);
		}
	}
