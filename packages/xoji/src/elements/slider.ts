...
export class XojiSlider extends XojiElement {
	...
	private elementId = `xoji-slider-${Math.random().toString(36).slice(2, 8)}`;
	...
	private get rail(): HTMLElement | null {
		return this.root.querySelector(".xoji-slider__rail");
	}

	private get thumb(): HTMLElement | null {
		return this.root.querySelector(".xoji-slider__thumb");
	}
	...
	private warnIfUnnamed(): void {
		if (!this.getAttribute("label") && !this.getAttribute("labelledby")) {
			console.warn(
				"xoji-slider: no accessible name. Provide a `label` or `labelledby` so the slider is announced.",
			);
		}
	}
	...
	private wirePointer(): void {
		...
		this.root.addEventListener("pointerdown", (e) => {
			...
			if (target?.closest(".xoji-slider__rail")) this.onPointerdown(e as PointerEvent);
		});
		this.root.addEventListener("dblclick", (e) => {
			const target = e.target as HTMLElement | null;
			if (!this.disabled && target?.closest(".xoji-slider__thumb")) {
				this.commit(this.defaultValue, "change");
			}
		});
		this.root.addEventListener("click", (e) => {
			const target = e.target as HTMLElement | null;
			if (this.editableValue && target?.closest(".xoji-slider__value")) this.enterValueEdit();
		});
	}
	...
	private enterValueEdit(): void {
		if (this.disabled) return;
		const valueSpan = this.root.querySelector(".xoji-slider__value") as HTMLElement | null;
		if (!valueSpan || valueSpan.querySelector("input")) return;
		const input = document.createElement("input");
		input.type = "text";
		input.inputMode = "decimal";
		input.className = "xoji-slider__value-input";
