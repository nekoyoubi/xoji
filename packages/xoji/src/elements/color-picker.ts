...
export class XojiColorPicker extends XojiElement {
	...
	private syncFormatButton(): void {
		const button = this.el(".xoji-color-picker__format");
	...
	private commitChannels(kind: "input" | "change"): void {
		...
		if (!model) return;
		const inputs = Array.from(this.root.querySelectorAll<HTMLInputElement>(".xoji-color-picker__channel-input"));
	...
	private renderPlane(): void {
		const canvas = this.el(".xoji-color-picker__plane") as HTMLCanvasElement | null;
	...
	private dragPlane(event: PointerEvent): void {
		const canvas = this.el(".xoji-color-picker__plane");
		if (!canvas) return;
		this.el(".xoji-color-picker__plane-handle")?.focus();
	...
	private paint(): void {
		...
		set(".xoji-color-picker__area", (el) => {
			el.style.backgroundColor = hexOpaque({ h: this.hsv.h, s: 1, v: 1 });
		});
		set(".xoji-color-picker__sv-handle", (el) => {
		...
		set(".xoji-color-picker__hue-handle", (el) => {
			el.style.insetInlineStart = `${((this.hsv.h / 360) * 100).toFixed(2)}%`;
			el.setAttribute("aria-valuenow", String(Math.round(this.hsv.h)));
		});
		set(".xoji-color-picker__alpha", (el) => el.style.setProperty("--cp-color", opaque));
		set(".xoji-color-picker__alpha-handle", (el) => {
		...
		set(".xoji-color-picker__swatch", (el) => el.style.setProperty("--cp-color", css));
		set(".xoji-color-picker__trigger", (el) => el.style.setProperty("--cp-color", css));
		this.root.querySelectorAll<HTMLElement>(".xoji-color-picker__preset").forEach((chip) => {
		...
		if (against) {
			...
			set(".xoji-color-picker__contrast-sample", (el) => {
				el.style.color = opaque;
				el.style.backgroundColor = againstHex;
			});
			set(".xoji-color-picker__contrast-ratio", (el) => {
				el.textContent = ratio.toFixed(2);
			});
			const grade = (level: string, threshold: number) =>
				set(`.xoji-color-picker__contrast-badge[data-level="${level}"]`, (el) => {
		...
		if (harmonyEl && this.harmonyScheme) {
			...
			harmonyEl.innerHTML = related
				.map((rc) => {
					...
					return `<button class="xoji-color-picker__preset xoji-color-picker__harmony-chip" part="harmony-chip" type="button" data-color="${hex}" data-hex="${hex}" style="--cp-chip: ${hex}" aria-label="${hex}"${this.disabled ? " disabled" : ""}></button>`;
				})
				.join("");
		}
		...
		if (channelModel) {
			...
			this.root.querySelectorAll<HTMLInputElement>(".xoji-color-picker__channel-input").forEach((input, i) => {
				const def = defs[i];
				...
				const readout = input.closest(".xoji-color-picker__channel")?.querySelector(".xoji-color-picker__channel-value");
		...
		if (this.planeEnabled) {
			...
			set(".xoji-color-picker__plane-handle", (el) => {
			...
			set(".xoji-color-picker__plane-readout", (el) => {
				el.textContent = `L ${l.toFixed(2)} · C ${c.toFixed(3)}`;
			});
		}
		const namedSnap = this.el(".xoji-color-picker__snap--named");
		if (namedSnap) {
		...
		const valueInput = this.el(".xoji-color-picker__value") as HTMLInputElement | null;
	...
	private dragArea(event: PointerEvent): void {
		const area = this.el(".xoji-color-picker__area");
		if (!area) return;
		this.el(".xoji-color-picker__sv-handle")?.focus();
		(event.target as Element).setPointerCapture?.(event.pointerId);
		const apply = (e: PointerEvent, kind: "input" | "change") =>
	...
	private wireEvents(): void {
		this.el(".xoji-color-picker__area")?.addEventListener("pointerdown", (e) => this.dragArea(e as PointerEvent));
		this.el(".xoji-color-picker__sv-handle")?.addEventListener("keydown", (e) => this.onSvKeydown(e as KeyboardEvent));
		this.el(".xoji-color-picker__hue")?.addEventListener("pointerdown", (e) =>
			this.dragTrack(e as PointerEvent, ".xoji-color-picker__hue", ".xoji-color-picker__hue-handle", (f, kind) => this.patchHsv({ h: f * 360 }, kind)),
		);
		this.el(".xoji-color-picker__hue-handle")?.addEventListener("keydown", (e) => this.onHueKeydown(e as KeyboardEvent));
		this.el(".xoji-color-picker__alpha")?.addEventListener("pointerdown", (e) =>
			this.dragTrack(e as PointerEvent, ".xoji-color-picker__alpha", ".xoji-color-picker__alpha-handle", (f, kind) => this.setAlpha(f, kind)),
		);
		this.el(".xoji-color-picker__alpha-handle")?.addEventListener("keydown", (e) => this.onAlphaKeydown(e as KeyboardEvent));
		const valueInput = this.el(".xoji-color-picker__value") as HTMLInputElement | null;
		...
		const trigger = this.el(".xoji-color-picker__trigger");
		const popover = this.el(".xoji-color-picker__popover");
		if (trigger && popover) {
		...
		this.el(".xoji-color-picker__plane")?.addEventListener("pointerdown", (e) => this.dragPlane(e as PointerEvent));
		this.el(".xoji-color-picker__plane-handle")?.addEventListener("keydown", (e) => this.onPlaneKeydown(e as KeyboardEvent));
		const channels = this.el(".xoji-color-picker__channels");
		channels?.addEventListener("input", () => this.commitChannels("input"));
		channels?.addEventListener("change", () => this.commitChannels("change"));
		this.el(".xoji-color-picker__snaps")?.addEventListener("click", (event) => {
			const button = (event.target as HTMLElement).closest<HTMLElement>(".xoji-color-picker__snap");
			if (button?.dataset.snap) this.applySnap(button.dataset.snap);
		});
		this.el(".xoji-color-picker__format")?.addEventListener("click", () => this.cycleFormat());
		this.el(".xoji-color-picker__eyedropper")?.addEventListener("click", () => void this.openEyeDropper());
		this.root.querySelectorAll<HTMLElement>(".xoji-color-picker__preset").forEach((chip) => {
		...
		this.el(".xoji-color-picker__harmony")?.addEventListener("click", (event) => {
			if (this.disabled) return;
			const chip = (event.target as HTMLElement).closest<HTMLElement>(".xoji-color-picker__harmony-chip");
	...
	private warnIfUnnamed(): void {
		if (!this.getAttribute("label") && !this.getAttribute("labelledby")) {
			console.warn(
				"xoji-color-picker: no accessible name. Provide a `label` or `labelledby` so the picker is announced.",
			);
		}
	}
	...
	private elementId = `xoji-color-picker-${Math.random().toString(36).slice(2, 8)}`;
}

define("xoji-color-picker", XojiColorPicker);
