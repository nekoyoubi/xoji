...
export class XojiSwatch extends XojiElement {
	...
	private detailsId = `xoji-swatch-details-${Math.random().toString(36).slice(2, 8)}`;
	...
	private repositionDetails = (): void => {
		...
		const swatch = this.root.querySelector<HTMLElement>(".xoji-swatch");
		const details = this.root.querySelector<HTMLElement>(".xoji-swatch__details");
