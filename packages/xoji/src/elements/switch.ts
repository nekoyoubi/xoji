...
export class XojiSwitch extends XojiElement {
	...
	private elementId = `xoji-switch-${Math.random().toString(36).slice(2, 8)}`;
	...
	private warnIfUnnamed(): void {
		...
		if (!hasName) {
			console.warn(
				"xoji-switch: no accessible name. Provide a `label`, `labelledby`, or `on-label`/`off-label` so the toggle is announced.",
			);
		}
	}
