...
export class XojiRadio extends XojiElement {
	...
	private warnIfUnnamed(): void {
		...
		if (!hasLabel && !this.getAttribute("aria-label")) {
			console.warn(
				"xoji-radio: no accessible name. Provide label text in the default slot, or a `label` / `labelledby` attribute.",
			);
		}
	}
...
export class XojiRadioGroup extends XojiElement {
	static get observedAttributes(): string[] {
	...
	private radios(): XojiRadio[] {
		const all = Array.from(this.querySelectorAll<XojiRadio>("xoji-radio"));
	...
	private onChange = (event: Event): void => {
		...
		const target = (event.target as Element | null)?.closest("xoji-radio") as XojiRadio | null;
		if (target?.checked) {
			for (const radio of Array.from(this.querySelectorAll<XojiRadio>("xoji-radio"))) {
	...
	private onKeydown = (event: KeyboardEvent): void => {
		...
		const radios = this.radios();
		if (radios.length === 0) return;
		const current = (event.target as Element | null)?.closest("xoji-radio") as XojiRadio | null;
	...
	private warnIfUnnamed(): void {
		if (!this.getAttribute("label") && !this.getAttribute("labelledby") && !this.getAttribute("aria-label")) {
			console.warn(
				"xoji-radio-group: no accessible name. Provide a `label`, `labelledby`, or `aria-label` so the group is announced.",
			);
		}
	}
	...
	private labelId = `xoji-radio-group-${Math.random().toString(36).slice(2, 8)}-label`;
...
define("xoji-radio", XojiRadio);
define("xoji-radio-group", XojiRadioGroup);
