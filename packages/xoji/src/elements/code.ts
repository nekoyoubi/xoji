 ...
 */
export class XojiCode extends XojiElement {
	...
	private withLines(html: string): string {
		...
		if (!this.lineNumbers && !spec) {
			this.style.removeProperty("--xoji-code-gutter");
			return html;
		}
		const split = splitCodeLines(html, spec);
		if (this.lineNumbers) this.style.setProperty("--xoji-code-gutter", codeGutterWidth(split.lines));
		else this.style.removeProperty("--xoji-code-gutter");
		return split.html;
	}
	...
	private seedScaffold(html: string, language: string | null): void {
		...
		if (pre instanceof HTMLElement) pre.className = `xoji-code ${languageClass}`;
	}
