...
export class XojiCheckbox extends XojiElement {
	...
	private warnIfUnnamed(): void {
		...
		if (!hasLabel) {
			console.warn(
				"xoji-checkbox: no accessible name. Provide label text in the default slot, or a `label` / `labelledby` attribute.",
			);
		}
	}
...
define("xoji-checkbox", XojiCheckbox);
 ...
 */
export class XojiCheckboxGroup extends XojiElement {
	private labelId = `xoji-checkbox-group-${Math.random().toString(36).slice(2, 8)}-label`;
	...
	private items(): XojiCheckbox[] {
		return Array.from(this.querySelectorAll<XojiCheckbox>("xoji-checkbox"));
	}

	private get headingInput(): HTMLInputElement | null {
		return this.root.querySelector(".xoji-checkbox-group__heading input");
	}

	private get headingLabel(): HTMLElement | null {
		return this.root.querySelector(".xoji-checkbox-group__heading");
	}
	...
	sync(): void {
		...
		heading.classList.toggle("xoji-checkbox--indeterminate", some);
	}
	...
	private onItemChange = (event: Event): void => {
		...
		// here. Only composed item changes (from `xoji-checkbox`) do, so this only ever rolls up.
		if (this.manual) return;
		if (!(event.target as Element | null)?.closest("xoji-checkbox")) return;
		this.sync();
	};
	...
	private onHeadingToggle = (): void => {
		...
		if (!this.manual) {
			...
			this.headingLabel?.classList.remove("xoji-checkbox--indeterminate");
		}
		this.dispatchEvent(new Event("change", { bubbles: true, composed: true }));
	};
	...
	private warnIfUnnamed(): void {
		if (!this.label && !this.getAttribute("labelledby") && !this.getAttribute("aria-label")) {
			console.warn(
				"xoji-checkbox-group: no accessible name. Provide a `label`, `labelledby`, or `aria-label` so the group is announced.",
			);
		}
	}
...
define("xoji-checkbox-group", XojiCheckboxGroup);
