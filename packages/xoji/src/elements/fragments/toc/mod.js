"use strict";
(() => {
  // packages/xoji/src/elements/fragments/toc/mod.ts
  ...
  function tocClass(bindings) {
    return ["xoji-toc", bindings.sticky && "xoji-toc--sticky"].filter(Boolean).join(" ");
  }
  function links(bindings) {
    const items = bindings.items ?? [];
    return items.map(
      (item) => `<li><a class="xoji-toc__link" part="link" href="#${escapeAttr(item.id)}" data-toc-link="${escapeAttr(item.id)}">${escapeHtml(item.label)}</a></li>`
    ).join("");
  }
