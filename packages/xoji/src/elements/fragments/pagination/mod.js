"use strict";
(() => {
  // packages/xoji/src/elements/fragments/pagination/mod.ts
  ...
  function paginationClass(bindings) {
    ...
    return ["xoji-pagination", `xoji-pagination--${tone}`, size !== "md" && `xoji-pagination--${size}`].filter(Boolean).join(" ");
  }
  ...
  function pageItem(n, current, link) {
    ...
    const cell = link !== void 0 ? `<a class="xoji-pagination__page" part="page" href="${escapeAttr(href(link, n))}" aria-label="${escapeAttr(label)}" data-page="${n}">${n}</a>` : `<button type="button" class="xoji-pagination__page" part="page" aria-label="${escapeAttr(label)}" data-page="${n}">${n}</button>`;
  ...
  function body(bindings) {
    ...
    const items = range(page, total, bindings.siblings ?? 1, bindings.boundaries ?? 1).map(
      (entry) => entry === "ellipsis" ? `<li class="xoji-pagination__item" part="item-wrap"><span class="xoji-pagination__ellipsis" part="ellipsis" aria-hidden="true">\u2026</span></li>` : pageItem(entry, page, link)
    ).join("");
