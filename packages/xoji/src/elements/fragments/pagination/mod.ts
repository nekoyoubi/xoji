...
function range(page: number, total: number, siblings: number, boundaries: number): (number | "ellipsis")[] {
...
function href(template: string, page: number): string {
	return template.includes("{page}") ? template.replace(/\{page\}/g, String(page)) : `${template}${page}`;
}

function paginationClass(bindings: PaginationBindings): string {
	const tone = bindings.tone ?? "accent";
	const size = bindings.size ?? "md";
	return ["xoji-pagination", `xoji-pagination--${tone}`, size !== "md" && `xoji-pagination--${size}`]
		.filter(Boolean)
		.join(" ");
}
...
function control(rel: "prev" | "next", target: number, disabled: boolean, link: string | undefined, ariaLabel: string): string {
	const glyph = rel === "prev" ? ARROW_PREV : ARROW_NEXT;
	const cls = `xoji-pagination__control xoji-pagination__control--${rel}`;
	if (link && !disabled) {
...
function pageItem(n: number, current: number, link: string | undefined): string {
	if (n === current) {
		return `<li class="xoji-pagination__item" part="item-wrap"><span class="xoji-pagination__page xoji-pagination__page--current" part="page" aria-current="page">${n}</span></li>`;
	}
	const label = `Go to page ${n}`;
	const cell =
		link !== undefined
			? `<a class="xoji-pagination__page" part="page" href="${escapeAttr(href(link, n))}" aria-label="${escapeAttr(label)}" data-page="${n}">${n}</a>`
			: `<button type="button" class="xoji-pagination__page" part="page" aria-label="${escapeAttr(label)}" data-page="${n}">${n}</button>`;
	return `<li class="xoji-pagination__item" part="item-wrap">${cell}</li>`;
}

function body(bindings: PaginationBindings): string {
	...
	const items = range(page, total, bindings.siblings ?? 1, bindings.boundaries ?? 1)
		.map((entry) =>
			entry === "ellipsis"
				? `<li class="xoji-pagination__item" part="item-wrap"><span class="xoji-pagination__ellipsis" part="ellipsis" aria-hidden="true">…</span></li>`
				: pageItem(entry, page, link),
		)
		.join("");
	...
	return `${prev}<ol class="xoji-pagination__list" part="list">${items}</ol>${next}`;
}
