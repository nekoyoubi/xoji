"use strict";
(() => {
  // packages/xoji/src/elements/fragments/grid/mod.ts
  ...
  function gridClass(b) {
    ...
    return [
      "xoji-grid",
      `xoji-grid--gap-${gap}`,
      usesColumns && `xoji-grid--cols-${columns}`,
      b.align && `xoji-grid--align-${b.align}`,
      b.justify && `xoji-grid--justify-${b.justify}`,
      b.inline && "xoji-grid--inline"
    ].filter(Boolean).join(" ");
  }
