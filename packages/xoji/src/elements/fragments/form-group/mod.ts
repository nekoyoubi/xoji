...
function groupClass(b: FormGroupBindings): string {
	...
	return ["xoji-form-group", b.invalid && "xoji-form-group--invalid", size !== "md" && `xoji-form-group--${size}`]
		.filter(Boolean)
		.join(" ");
}
...
function inner(b: FormGroupBindings): string {
	...
	return (
		`<label class="xoji-form-group__label" part="label" data-label id="${b.labelId ?? ""}"${labelFor}${labelHidden}>` +
		`<span data-label-text>${escape(label)}</span>` +
		`<span class="xoji-form-group__required" part="required-indicator" data-required aria-hidden="true"${requiredHidden}>*</span>` +
		`</label>` +
		`<span class="xoji-form-group__description" part="description" data-description id="${b.descriptionId ?? ""}"${descriptionHidden}>${escape(description)}</span>` +
		`<div class="xoji-form-group__control" part="control"><slot></slot></div>` +
		`<span class="xoji-form-group__error" part="error" data-error id="${b.errorId ?? ""}" role="alert"${errorHidden}>${escape(error)}</span>`
	);
}
