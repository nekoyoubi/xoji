...
function inner(b: DialogBindings): string {
	...
	const closeButton = showCloseButton
		? `<button type="button" class="xoji-dialog__close" part="close" aria-label="${closeLabel}">${CLOSE_ICON}</button>`
		: "";
	...
	return (
		...
		header +
		`<div class="xoji-dialog__body" part="body"><slot></slot></div>` +
		`<footer class="xoji-dialog__footer" part="footer"><slot name="footer"></slot></footer>` +
		`</dialog>`
	);
}
