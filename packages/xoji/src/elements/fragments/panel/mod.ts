...
const MARKER =
	'<span class="xoji-panel__marker" part="marker" aria-hidden="true">' +
...
const ACTIONS_SLOT = `<span class="xoji-slot" data-slot="actions"><slot name="actions"></slot></span>`;
...
function panelClass(b: PanelBindings): string {
	...
	return ["xoji-panel", variant !== "default" && `xoji-panel--${variant}`, b.scrollable && "xoji-panel--scroll"]
		.filter(Boolean)
		.join(" ");
}
...
function body(b: PanelBindings): string {
	const bodyAttrs = b.scrollable
		...
		: "";
	return (
		`<div class="xoji-panel__body" part="body"${bodyAttrs}><slot></slot></div>` +
		`<div class="xoji-panel__footer" part="footer"><slot name="footer"></slot></div>`
	);
}
...
function inner(b: PanelBindings): string {
	...
	if (isCollapsible(b)) {
		...
		return (
			`<div class="xoji-panel__header xoji-panel__header--toggle" part="header">` +
			`<button class="xoji-panel__toggle" part="toggle" type="button" aria-expanded="${expanded}" ` +
			`aria-controls="${uid}-region">${MARKER}` +
			`<span class="xoji-panel__title" part="title" id="${uid}">${b.title ?? ""}</span></button>` +
			`${ACTIONS_SLOT}</div>` +
			`<div class="xoji-panel__collapse" part="collapse" id="${uid}-region" role="region" ` +
			`aria-labelledby="${uid}"${b.open ? "" : " hidden"}>${body(b)}</div>`
		);
	}
	const tag = `h${level(b)}`;
	const heading = b.title
		? `<${tag} class="xoji-panel__title" part="title" id="${uid}">${b.title}</${tag}>`
		: "";
	const header = hasHeader(b)
		? `<header class="xoji-panel__header" part="header">${heading}` +
			`<span class="xoji-panel__spacer" part="spacer"></span>${ACTIONS_SLOT}</header>`
		: "";
