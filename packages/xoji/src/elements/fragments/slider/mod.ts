...
function inner(b: SliderBindings): string {
	...
	const valueMarkup = b.showValue
		? `<span class="xoji-slider__value" part="value" aria-hidden="true">${readout(b, value)}</span>`
		: "";

	const headerLabel = b.hideLabel ? "" : label;
	const header = valueMarkup
		? `<span class="xoji-slider__header" part="header">${headerLabel}${valueMarkup}</span>${b.hideLabel ? label : ""}`
		: label;

	return (
		`${header}<span class="xoji-slider__rail" part="rail">` +
		`<span class="xoji-slider__fill" part="fill" style="width: ${pct}"></span>` +
		`<span class="xoji-slider__thumb" part="thumb" role="slider" tabindex="${tabindex}" ` +
