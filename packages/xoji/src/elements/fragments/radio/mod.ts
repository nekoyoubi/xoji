...
function inner(b: RadioBindings): string {
	...
	return (
		`<input part="control" class="xoji-radio__control" type="radio"` +
		`${nameAttr}${valueAttr}${checkedAttr}${disabledAttr}${ariaInvalid}${ariaLabel}${ariaLabelledby} />` +
		`<span part="indicator" class="xoji-radio__indicator" aria-hidden="true"></span>` +
		`<span part="label" class="xoji-radio__label"><slot>${labelText}</slot></span>`
	);
}
