...
const CHEVRON =
	'<svg class="xoji-accordion__chevron" viewBox="0 0 24 24" aria-hidden="true">' +
...
function items(bindings: AccordionBindings): string {
	...
	return sections
		.map((section, i) => {
			...
			return (
				`<div class="xoji-accordion__item${isOpen ? " is-open" : ""}" part="item" data-key="${key}">` +
				`<h${level} class="xoji-accordion__heading" part="heading">` +
				`<button class="xoji-accordion__trigger" part="trigger" type="button" id="${triggerId}" ` +
				`data-key="${key}" aria-expanded="${String(isOpen)}" aria-controls="${panelId}"${disabledAttr}>` +
				`<span class="xoji-accordion__label">${section.header}</span>${CHEVRON}</button></h${level}>` +
				`<div class="xoji-accordion__panel" part="panel" id="${panelId}" data-key="${key}" role="region" ` +
				`aria-labelledby="${triggerId}"${isOpen ? "" : " hidden"}>` +
				`<div class="xoji-accordion__content">${body}</div></div></div>`
			);
		})
		.join("");
}
