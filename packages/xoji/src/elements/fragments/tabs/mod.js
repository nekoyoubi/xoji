"use strict";
(() => {
  // packages/xoji/src/elements/fragments/tabs/mod.ts
  ...
  function rootClass(bindings) {
    ...
    const parts = ["xoji-tabs", `xoji-tabs--${variant}`];
    if (size !== "md") parts.push(`xoji-tabs--${size}`);
    if (bindings.sticky) parts.push("xoji-tabs--sticky");
    return parts.join(" ");
  }
  function tabButtons(bindings, selected) {
    const tabs = bindings.tabs ?? [];
    const uid = bindings.uid ?? "xoji-tabs";
  ...
  function panels(bindings, selected) {
    ...
    return tabs.map((tab, i) => {
      ...
      return `<div class="xoji-tabs__panel" part="panel" role="tabpanel" id="${uid}-panel-${i}" data-key="${key}" aria-labelledby="${uid}-tab-${i}" tabindex="0"${hidden}>${body}</div>`;
    }).join("");
  }
