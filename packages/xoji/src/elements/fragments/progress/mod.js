"use strict";
(() => {
  // packages/xoji/src/elements/fragments/progress/mod.ts
  var RADIUS = 16;
  ...
  function progressClass(b) {
    ...
    return [
      "xoji-progress",
      `xoji-progress--${variant}`,
      `xoji-progress--${tone}`,
      size !== "md" && `xoji-progress--${size}`,
      b.indeterminate && "xoji-progress--indeterminate",
      b.colorizeValue && "xoji-progress--colorize-value",
      b.valuePosition === "inset" && "xoji-progress--value-inset",
      pulse && `xoji-progress--pulse-${pulse}`
    ].filter(Boolean).join(" ");
  }
  function fraction(b) {
    const min = b.min ?? 0;
  ...
  function valueReadout(b) {
    return `<span class="xoji-progress__value" part="value"><slot name="value"><span data-progress-value>${valueText(b)}</span></slot></span>`;
  }
  ...
  hooks.fragment.update("progress", (bindings, ops) => {
    ...
    if ((bindings.variant ?? "linear") === "circular") {
      const offset = bindings.indeterminate ? (CIRCUMFERENCE * 0.7).toFixed(3) : (CIRCUMFERENCE * (1 - fraction(bindings))).toFixed(3);
      const dasharray = CIRCUMFERENCE.toFixed(3);
