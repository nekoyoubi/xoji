"use strict";
(() => {
  // packages/xoji/src/elements/fragments/statusbar/mod.ts
  function statusbarClass(b) {
    const overflow = b.overflow ?? "clip";
    return [
      "xoji-statusbar",
      overflow !== "clip" && `xoji-statusbar--${overflow}`,
      b.separated && "xoji-statusbar--separated"
    ].filter(Boolean).join(" ");
  }
  function overflowMenu(b) {
    const overflow = b.overflow ?? "clip";
    if (overflow !== "collapse" || b.manualOverflow) return "";
    const popoverId = `${b.elementId ?? "xoji-statusbar"}-overflow`;
    return `<span class="xoji-statusbar__overflow" part="overflow" style="display:none"><button class="xoji-statusbar__item xoji-statusbar__overflow-trigger" part="overflow-trigger" type="button" popovertarget="${popoverId}" aria-label="Show hidden status items">+0</button><div class="xoji-statusbar__overflow-popover" part="overflow-popover" id="${popoverId}" popover role="group" aria-label="Hidden status items"></div></span>`;
  }
