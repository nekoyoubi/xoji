"use strict";
(() => {
  // packages/xoji/src/elements/fragments/field/mod.ts
  ...
  function inner(b) {
    ...
    const star = required ? `<span class="xoji-field__required" part="required" aria-hidden="true">*</span>` : "";
    ...
    return `<label class="xoji-field__label" part="label" for="${escapeAttr(inputId)}"${labelHidden}>${escapeHtml(label)}${star}</label><div class="xoji-field__control" part="control"><span class="xoji-field__adornment" part="adornment"><slot name="prefix"></slot></span><input ${inputAttrs} /><button type="button" class="xoji-field__action" part="action-reveal" data-action="reveal"${revealHidden} aria-pressed="${revealPressed}" aria-label="${escapeAttr(revealLabel)}"><slot name="reveal-icon"><span aria-hidden="true">&#128065;</span></slot></button><button type="button" class="xoji-field__action" part="action-clear" data-action="clear"${clearHidden} aria-label="Clear"><slot name="clear-icon"><span aria-hidden="true">&times;</span></slot></button><span class="xoji-field__adornment" part="adornment"><slot name="suffix"></slot></span></div><span class="xoji-field__description" part="description" id="${escapeAttr(descriptionId)}"${descriptionHidden}>${escapeHtml(description)}</span><span class="xoji-field__error" part="error" id="${escapeAttr(errorId)}"${errorHidden}>${escapeHtml(error)}</span>`;
  }
  ...
  hooks.fragment.update("field", (bindings, ops) => {
    ...
    const star = bindings.required ? `<span class="xoji-field__required" part="required" aria-hidden="true">*</span>` : "";
