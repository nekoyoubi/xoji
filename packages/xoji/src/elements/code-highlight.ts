 ...
 * `preload` attribute and `XojiCode.warm()` both feed. Unknown ids are skipped,
...
export interface SupportedLanguages {
	...
	canonical: string[];
	/** Languages xoji adds that Prism does not ship (e.g. `svelte`, `astro`). */
	custom: string[];
...
/**
 * The full set of languages `<xoji-code>` can highlight: the canonical Prism ids,
 * the custom built-ins xoji adds, and the alias table. Pure data, no grammar load —
