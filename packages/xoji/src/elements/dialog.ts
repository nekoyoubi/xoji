...
export class XojiDialog extends XojiElement {
	protected override get styleMode(): StyleMode {
		return "auto";
	}

	private elementId = `xoji-dialog-${Math.random().toString(36).slice(2, 8)}`;
	...
	private warnIfUnnamed(): void {
		if (!this.getAttribute("heading") && !this.getAttribute("labelledby") && !this.getAttribute("label")) {
			console.warn(
				"xoji-dialog: no accessible name. Provide a `heading`, `label`, or `labelledby` so the dialog is announced.",
			);
		}
	}
