...
export class XojiAccordion extends XojiElement {
	protected override get styleMode(): StyleMode {
		return "auto";
	}

	private uid = `xoji-accordion-${accordionSeq++}`;
	...
	get items(): AccordionSection[] {
		...
		try {
			...
			console.warn("xoji-accordion: `items` is not valid JSON; reading the light-DOM sections instead.");
			return [];
		}
	}
	...
	private applyIntent(intent: FragmentIntent, event: Event): void {
		...
		if (intent.focus !== undefined) {
			const trigger = this.root.querySelector<HTMLElement>(
				`.xoji-accordion__trigger[data-key="${CSS.escape(intent.focus)}"]`,
			);
			trigger?.focus();
		}
...
define("xoji-accordion", XojiAccordion);
