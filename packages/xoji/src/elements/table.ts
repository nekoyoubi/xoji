...
const SORT_GLYPH = `<span class="xoji-table__sort" aria-hidden="true"><svg viewBox="0 0 24 24" width="1em" height="1em"><path fill="currentColor" d="M12 8l5 6H7l5-6Z" /></svg></span>`;

export class XojiTable extends HTMLElement {
	...
	private get computedTableClass(): string[] {
		return [
			"xoji-table",
			this.variant !== "default" && `xoji-table--${this.variant}`,
			this.size === "compact" && "xoji-table--compact",
			this.hover && "xoji-table--hover",
			this.sticky && "xoji-table--sticky",
	...
	private decorate(): void {
		...
		if (maxHeight) this.style.setProperty("--xoji-table-max-height", maxHeight);
		else this.style.removeProperty("--xoji-table-max-height");
		this.updateScrollAffordance();

		const table = this.querySelector("table");
		if (!table) {
			console.warn("xoji-table: no `<table>` child to style. Author a real table and the wrapper styles it.");
			return;
		}

		table.classList.add("xoji-table");
		...
		if (!table.getAttribute("aria-label") && !table.querySelector("caption") && !this.getAttribute("aria-label")) {
			console.warn(
				"xoji-table: no `<caption>` or `aria-label` — the table has no accessible name. Add one so it is announced.",
			);
		}
		...
		table.querySelector("thead")?.classList.add("xoji-table__head");
		table.querySelector("tbody")?.classList.add("xoji-table__body");
		for (const row of Array.from(table.querySelectorAll("tr"))) row.classList.add("xoji-table__row");
		for (const cell of Array.from(table.querySelectorAll("td"))) cell.classList.add("xoji-table__cell");
		for (const header of Array.from(table.querySelectorAll("th"))) {
			header.classList.add("xoji-table__header-cell");
			this.decorateHeader(header);
		}
		for (const caption of Array.from(table.querySelectorAll("caption"))) {
			caption.classList.add("xoji-table__caption");
		}
		for (const footCell of Array.from(table.querySelectorAll("tfoot td, tfoot th"))) {
			footCell.classList.remove("xoji-table__cell", "xoji-table__header-cell");
			footCell.classList.add("xoji-table__footer-cell");
		}
	}
	...
	private decorateHeader(header: Element): void {
		...
		if (!sortable) return;
		header.classList.add("xoji-table__header-cell--sortable");
		if (!header.hasAttribute("tabindex")) header.setAttribute("tabindex", "0");
		if (!header.hasAttribute("aria-sort")) header.setAttribute("aria-sort", "none");
		if (!header.querySelector(".xoji-table__header-content")) {
			const content = document.createElement("span");
			content.className = "xoji-table__header-content";
