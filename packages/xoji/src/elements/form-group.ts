...
export class XojiFormGroup extends XojiElement {
	private groupNumber = ++formGroupCounter;
	private labelId = `xoji-form-group-label-${this.groupNumber}`;
	private controlId = `xoji-form-group-control-${this.groupNumber}`;
	private descriptionId = `xoji-form-group-desc-${this.groupNumber}`;
	...
	private isControlLike(el: HTMLElement): boolean {
		...
		return tag.startsWith("xoji-") || el.hasAttribute("role");
	}
	...
	private warnIfUnlabelled(): void {
		if (this.label.length === 0) {
			console.warn(
				"xoji-form-group: no `label` was provided. The slotted control may lack an accessible name; provide a `label` or label the control directly.",
			);
		}
	}
...
define("xoji-form-group", XojiFormGroup);
