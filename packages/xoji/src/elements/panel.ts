...
export class XojiPanel extends XojiElement {
	private titleId = `xoji-panel-title-${panelTitleSeq++}`;
	...
	private warnIfUnnamed(): void {
		if (!this.hasName) {
			console.warn(
				"xoji-panel: no title, actions, or label — the panel has no accessible name. Provide a `title` (visible heading) or `label` (name only) so the region is announced.",
			);
		}
	}
