...
export class XojiTabs extends XojiElement {
	protected override get styleMode(): StyleMode {
		return "auto";
	}

	private uid = `xoji-tabs-${tabsSeq++}`;
	...
	get items(): TabItemData[] {
		...
		try {
			...
			console.warn("xoji-tabs: `items` is not valid JSON; reading the light-DOM tabs instead.");
			return [];
		}
	}
	...
	private warnIfUnnamed(): void {
		if (!this.getAttribute("label") && !this.getAttribute("labelledby")) {
			console.warn(
				"xoji-tabs: no accessible name. Provide a `label` or `labelledby` so the tablist is announced.",
			);
		}
	}
