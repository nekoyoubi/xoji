...
export class XojiDock extends XojiElement {
	...
	private warnIfUnnamed(): void {
		if (!this.getAttribute("label") && !this.getAttribute("aria-label") && !this.getAttribute("aria-labelledby")) {
			console.warn(
				"xoji-dock: a dock has no accessible name. Provide a `label` or `aria-label` so multiple docks are distinguishable.",
			);
		}
	}
