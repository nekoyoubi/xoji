...
/**
 * The base every xoji element extends — and the blessed base for consumer
 * subclasses too. The `protected` members below are the stable extension contract:
 * extend a shipped element (e.g. `XojiButton`) to add behavior or state, or extend
 * `XojiElement` directly for a fully custom element that still rides the shared token
...
export abstract class XojiElement extends HTMLElement {
	...
	constructor() {
		...
		// nested auto component (e.g. a `<xoji-text>` inside a slotted panel) carries its own deeper
	...
	/** Per-element host-layout rules only (e.g. `:host { display: ... }`). The component's visual styling comes from the shared `@xoji/core/css` sheet. */
