 ...
 * (`[data-slot-fallback]`) and the `<!--xoji:slot-->` composition marker comment. So `hasSlotted`
...
export class FragmentHost {
	...
	private apply(loaded: LoadedFill, bindings: Record<string, unknown>): void {
		...
		// the fallback stays and only the `<!--xoji:slot-->` composition marker is stripped (so an
			...
			for (const region of this.root.querySelectorAll("[data-slot]")) {
				...
				if (nodes?.length) {
					...
					for (const node of [...region.childNodes]) {
						if (node.nodeType === 8 && /^xoji:slot/.test(node.textContent ?? "")) region.removeChild(node);
					}
				}
			}
		}
