 ...
 * `@xoji/core/authoring`, calls one, and the helper registers the `graph` /
...
/* ── tier 1: a xoji-family preset ──────────────────────────────────────────
 * Declare taste; the shared xoji derivation does the rest. `xoji-default` is
 * the default vector, so its whole definition is `{ id: "xoji-default" }`.
 */

export interface XojiAlgorithmSpec {
	id: string;
	 ...
	 * (the shared xoji `settle` derivation). A multi-pass family algorithm typically
	 * begins with `settle` and layers register→register transforms after it.
	 */
	passes?: (preset: PresetDefaults, input: DeriveOptions) => Pass[];
}

export function toPreset(spec: XojiAlgorithmSpec): PresetDefaults {
...
export function defineXojiAlgorithm(spec: XojiAlgorithmSpec): void {
