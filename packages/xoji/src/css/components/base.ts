 ...
 * xoji app wants, plus `.xoji-control` — the common input chrome that Field,
 * Select, and Textarea inherit. Concatenated FIRST in `componentsCss` so the
 * resets cascade correctly and component modules can rely on `.xoji-control`.
 *
...
.xoji-control::placeholder { color: var(--placeholder); }
.xoji-control:focus-visible {
	outline: var(--border-normal) solid transparent;
	border-color: var(--accent);
	box-shadow: 0 0 0 var(--border-normal) var(--ring);
}
.xoji-control:disabled {
	cursor: not-allowed;
	color: var(--fg-disabled);
	background: var(--state-disabled);
}
.xoji-control--invalid {
