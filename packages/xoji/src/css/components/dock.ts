...
const dockToneEdges = FULL_TONES.map(
	(t) => `.xoji-dock--${t} { --dock-edge: var(--${t}); --dock-edge-w: var(--border-thick); }`,
).join("\n");
...
${dockToneEdges}
.xoji-dock--edge-thin { --dock-edge-w: var(--border-thin); }
.xoji-dock--edge-thick { --dock-edge-w: var(--border-thick); }
.xoji-dock--edge-bold { --dock-edge-w: var(--space-1); }
.xoji-dock--edge-out {
	border-right: none;
	border-left: var(--dock-edge-w, var(--border-thin)) solid var(--dock-edge, var(--line));
}
.xoji-dock--right.xoji-dock--edge-out {
	border-left: none;
	border-right: var(--dock-edge-w, var(--border-thin)) solid var(--dock-edge, var(--line));
}
`.trim();
