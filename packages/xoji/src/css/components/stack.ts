...
const gapRules = GAPS.map((g) => `.xoji-stack--gap-${g} { gap: var(--space-${g}); }`).join("\n");
const alignRules = ALIGN.map((a) => `.xoji-stack--align-${a} { align-items: ${ALIGN_VALUE[a]}; }`).join("\n");
const justifyRules = JUSTIFY.map((j) => `.xoji-stack--justify-${j} { justify-content: ${JUSTIFY_VALUE[j]}; }`).join("\n");
