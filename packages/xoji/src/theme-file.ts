...
export const THEME_FILE_FORMAT = "xoji-theme" as const;
export const THEME_FILE_VERSION = 1 as const;
export const THEME_FILE_SCHEMA_URL = "https://xoji.dev/schema/theme.v1.json";
...
/**
 * The canonical, self-describing xoji theme artifact. `recipe` is the source of
 ...
 */
export interface XojiThemeFile {
	$schema?: string;
...
export function serializeThemeFile(file: XojiThemeFile): string {
	return `${JSON.stringify(file, null, 2)}\n`;
}

/** A strict check for the canonical envelope — format marker plus a usable recipe. */
export function isThemeFile(value: unknown): value is XojiThemeFile {
	return (
		isObject(value) &&
 ...
 * only the `xoji-theme` envelope. Looser coercion (bare recipes, raw token maps,
 * legacy docs) is a front-end concern, since only the front end knows its own input model.
 */
export function parseThemeFile(input: string | unknown): XojiThemeFile | null {
