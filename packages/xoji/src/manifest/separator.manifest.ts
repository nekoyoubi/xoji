...
const htmlExample = `<xoji-separator></xoji-separator>
...
export const separatorManifest: ComponentManifest = {
	...
	anatomy: [
		...
		},
		{
			...
			selector: ".xoji-separator__label",
		},
	],
	props: [
		{
	...
	variants: [
		{
			...
			className: "xoji-separator",
			tokens: ["--line"],
		},
		{
			name: "with-label",
			description: "Centers a label over the rule, splitting it into two flanking segments; promoted to `role=\"separator\"`.",
			className: "xoji-separator--with-label",
			tokens: ["--line", "--fg-2", "--space-3"],
		},
	],
	sizes: [
		{ name: "thin", description: "Faint: drawn in the lighter `--line-2`.", className: "xoji-separator--thin" },
		{ name: "normal", description: "Default: the standard `--line` weight.", className: "xoji-separator", isDefault: true },
	],
	states: [
		{
			name: "vertical",
			description: "Vertical orientation: the rule spans height instead of width, for inline groups.",
			selector: ".xoji-separator--vertical",
			tokens: ["--border-thin"],
		},
	],
	slots: [
