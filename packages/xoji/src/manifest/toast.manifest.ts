...
const htmlExample = `<xoji-toast-region id="toasts" placement="bottom-right"></xoji-toast-region>
...
export const toastManifest: ComponentManifest = {
	...
	anatomy: [
		...
		{
			name: "icon",
			description: "The severity glyph, shown when a `severity` is set, drawn in the card color (soft) or the on-fill ink (solid). Project an `icon` slot to override it (declarative `xoji-toast` / `Toast`).",
			selector: ".xoji-toast__icon",
			tokens: ["--text-lg"],
		},
		{
			name: "message",
			description: "The notification text; the content that gets announced.",
			selector: ".xoji-toast__message",
			tokens: ["--weight-medium"],
		},
		{
			name: "action",
			description: "An optional inline action button that runs a callback then dismisses the toast.",
			selector: ".xoji-toast__action",
			tokens: [
				"--text-sm",
				"--weight-semibold",
				"--leading-tight",
	...
	states: [
		...
		},
		{
			...
			selector: ".xoji-toast__action:focus-visible, .xoji-toast__close:focus-visible",
	...
	slots: [
		{
			name: "default",
			description: "The toast message (declarative `xoji-toast` / `Toast`).",
			bindings: ["html", "svelte", "astro"],
		},
		{
			name: "icon",
