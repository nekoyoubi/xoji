...
const htmlExample = `<xoji-select label="Theme" name="theme" value="auto">
...
export const selectManifest: ComponentManifest = {
	...
	anatomy: [
		...
		},
		{
			...
			selector: ".xoji-select__control",
		},
		{
			name: "select",
			description: "The native `<select>` carrying the shared `.xoji-control` chrome with the platform arrow suppressed.",
			selector: ".xoji-select__field",
			tokens: ["--space-7"],
		},
		{
			name: "chevron",
			description: "The custom dropdown indicator, decorative and pointer-transparent, recolored by state.",
			selector: ".xoji-select__chevron",
	...
	variants: [
		{
			name: "default",
			description: "The standard control: `.xoji-control` chrome with the accent focus ring.",
			className: "xoji-select",
		},
		{
			name: "invalid",
			description: "Validation-failed treatment: danger border and chevron, danger-tinted focus ring.",
			className: "xoji-select--invalid",
			tokens: ["--danger", "--danger-bg", "--border-normal"],
		},
	],
	sizes: [
		{ name: "sm", description: "Compact.", className: "xoji-select--sm" },
		{ name: "md", description: "Default.", className: "xoji-select", isDefault: true },
		{ name: "lg", description: "Large.", className: "xoji-select--lg" },
	],
	states: [
		{
			name: "focus-visible",
			description: "Keyboard focus: the `.xoji-control` accent ring, with the chevron tinted to match.",
			selector: ".xoji-select__field:focus-visible + .xoji-select__chevron",
			tokens: ["--accent"],
		},
		{
			name: "invalid-focus",
			description: "Focus while invalid: the danger border holds and the ring shifts to the danger tint.",
			selector: ".xoji-select--invalid .xoji-select__field:focus-visible",
			tokens: ["--danger", "--danger-bg", "--border-normal"],
		},
		{
			name: "disabled",
			description: "Non-interactive: the native control mutes and the chevron drops to the disabled ink.",
			selector: ".xoji-select__field:disabled + .xoji-select__chevron",
			tokens: ["--fg-disabled"],
		},
	],
	slots: [
