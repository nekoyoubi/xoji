...
export const cardManifest: ComponentManifest = {
	...
	description:
		"Card is the workhorse surface: a bordered, elevated panel that frames related content as one unit. It lays its three regions (header, body, footer) out in a column with consistent spacing; the header and footer are real light-DOM parts (`.xoji-card__header` / `.xoji-card__footer`), so the same structure renders identically across every binding. An `overlay` variant swaps the surface for the translucent treatment used inside popovers and menus; a `compact` size tightens the padding; an `interactive` variant adds a hover elevation lift plus a focus-within ring for cards that act as a single clickable target. That last one is presentational only: it styles the surface but adds no behavior, so the consumer wraps the clickable content in a real `<button>` or `<a>` for keyboard and screen-reader semantics.",
	...
	variants: [
		...
		},
		{
			...
			className: "xoji-card--interactive",
			tokens: ["--elevation-4", "--line-2", "--state-hover", "--state-press", "--ring"],
		},
	],
	sizes: [
		{ name: "default", description: "Standard padding.", className: "xoji-card", isDefault: true },
		{ name: "compact", description: "Tightened padding and spacing.", className: "xoji-card--compact" },
	],
	states: [
		{
			name: "hover",
			description: "Pointer over an interactive card; the surface lifts and the overlay paints the hover tint.",
			selector: ".xoji-card--interactive:hover",
		...
		},
		{
			...
			selector: ".xoji-card--interactive:active",
			tokens: ["--state-press"],
		},
		{
			name: "focus-within",
			...
			selector: ".xoji-card--interactive:focus-within",
