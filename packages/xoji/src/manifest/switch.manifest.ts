...
const htmlExample = `<xoji-switch label="Wi-Fi" checked></xoji-switch>
...
export const switchManifest: ComponentManifest = {
	...
	variants: [
		{ name: "square", description: "Squared track and thumb corners.", className: "xoji-switch--square" },
		{ name: "vertical", description: "Track stood on its end; down is off, up is on.", className: "xoji-switch--vertical" },
		{ name: "reverse", description: "On/off direction flipped (on at the left, or the bottom when vertical).", className: "xoji-switch--reverse" },
		{ name: "label-end", description: "Label after the control instead of before it.", className: "xoji-switch--label-end" },
	],
	sizes: [
		{ name: "sm", description: "Compact.", className: "xoji-switch--sm" },
		{ name: "md", description: "Default.", className: "xoji-switch", isDefault: true },
	],
	states: [
		{
			name: "on",
			description: "Toggle is on: the track fills with the chosen tone (accent by default) and the thumb slides to the end.",
			selector: '.xoji-switch__track[aria-checked="true"]',
			tokens: ["--accent", "--accent-fg"],
		},
		{
			name: "hover",
			description: "Pointer over the track: overlay paints the hover tint.",
			selector: ".xoji-switch__track:hover::after",
			tokens: ["--state-hover"],
		},
		{
			name: "active",
			description: "Track pressed: overlay paints the press tint.",
			selector: ".xoji-switch__track:active::after",
			tokens: ["--state-press"],
		},
		{
			name: "focus-visible",
			description: "Keyboard focus: a token-colored ring, plus a transparent outline that becomes real in forced-colors mode.",
			selector: ".xoji-switch__track:focus-visible",
			tokens: ["--border-normal", "--border-thick", "--ring"],
		},
		{
			name: "disabled",
			description: "Non-interactive: muted track and thumb, overlay suppressed.",
			selector: '.xoji-switch__track:disabled, .xoji-switch__track[aria-disabled="true"]',
