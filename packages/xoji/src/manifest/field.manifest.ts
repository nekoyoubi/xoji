...
const htmlExample = `<xoji-field label="Email" name="email" type="email" placeholder="you@example.com" required></xoji-field>
...
export const fieldManifest: ComponentManifest = {
	...
	anatomy: [
		...
		},
		{
			...
			selector: ".xoji-field__control",
			tokens: ["--field-bg", "--field-border", "--border-thin", "--radius-md", "--accent", "--ring", "--border-normal"],
		},
		{
			name: "input",
			description: "The native input, styled by `.xoji-control` with its own chrome neutralized so the control box owns the border.",
			selector: ".xoji-field__input",
		},
		{
			name: "adornment",
			description: "A leading (prefix) or trailing (suffix) slot for icons, currency symbols, or units.",
			selector: ".xoji-field__adornment",
