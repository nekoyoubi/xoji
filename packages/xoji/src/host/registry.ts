...
const MOD_DIRS: Record<string, string> = {
	"xoji-default": "xoji-default",
	"xoji-hc": "xoji-hc",
	"xoji-quiet": "xoji-quiet",
	"xoji-loud": "xoji-loud",
	"nxi-nite": "nxi-nite",
};
...
function findAlgorithmsRoot(): string {
	...
	while (true) {
		...
		if (existsSync(join(candidate, "xoji-default", "mod-manifest.json"))) {
			return candidate;
		}
		if (dir === root) break;
		dir = dirname(dir);
	}
	throw new Error("xoji: could not locate the algorithms/ directory");
}
 ...
 * `getAlgorithm` registry from `@xoji/core/algorithms` remains as the byte-identical
...
export function resolveAlgorithm(id: string): Promise<Algorithm> {
	...
	if (!dir) {
		return Promise.reject(
			new Error(`xoji: no hosted mod for algorithm "${id}" (have: ${Object.keys(MOD_DIRS).join(", ")})`),
		);
	}
