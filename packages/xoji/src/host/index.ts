...
function hostPassRunUnsupported(): never {
	throw new Error(
		"xoji: host passes are name-only; pass closures do not cross the sandbox — use deriveTraced for layer snapshots",
	);
}
 ...
 * import-free: it calls `defineAlgorithm` / `defineXojiAlgorithm` (Tier-2 or Tier-1) directly,
...
/**
 * Loads a xoji algorithm packaged as a script-format xript mod and returns a
