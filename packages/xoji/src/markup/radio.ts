...
export function radioGroupClass(props: RadioGroupMarkupProps): string {
	return ["xoji-radio-group", props.orientation === "horizontal" && "xoji-radio-group--horizontal"]
		.filter(Boolean)
		.join(" ");
}
...
export function radioGroupMarkup(props: RadioGroupMarkupProps): string {
	...
	const legend = label
		? `<span part="group" class="xoji-radio-group__label" id="${props.labelId}">${label}</span>`
		: "";
	let nameAttr = "";
