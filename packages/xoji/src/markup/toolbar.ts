...
export function toolbarClass(props: ToolbarMarkupProps): string {
	...
	return [
		"xoji-toolbar",
		size !== "md" && `xoji-toolbar--${size}`,
		props.sticky && "xoji-toolbar--sticky",
		props.bare && "xoji-toolbar--bare",
	]
		.filter(Boolean)
		.join(" ");
}
...
export function toolbarMarkup(props: ToolbarMarkupProps): string {
	...
	const inner = `<div class="xoji-toolbar__group xoji-toolbar__group--start" part="group"><slot name="start"></slot></div>${toolbarTitleMarkup(props)}<slot></slot><div class="xoji-toolbar__group xoji-toolbar__group--center" part="group"><slot name="center"></slot></div><div class="xoji-toolbar__group xoji-toolbar__group--end" part="group"><slot name="end"></slot></div>`;
