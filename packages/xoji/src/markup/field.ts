...
export function fieldClass(props: FieldMarkupProps): string {
	...
	return [
		"xoji-field",
		props.invalid && "xoji-field--invalid",
		props.disabled && "xoji-field--disabled",
		props.readonly && "xoji-field--readonly",
...
export function fieldMarkup(props: FieldMarkupProps): string {
	...
	return `
				<div class="${fieldClass(props)}" part="field">
					<label class="xoji-field__label" part="label" for="${escapeAttr(props.inputId)}"${labelHidden}>${escape(label)}${star}</label>
					...
					<span class="xoji-field__description" part="description" id="${escapeAttr(props.descriptionId)}"${descriptionHidden}>${escape(description)}</span>
					<span class="xoji-field__error" part="error" id="${escapeAttr(props.errorId)}"${errorHidden}>${escape(error)}</span>
				</div>
			`;
}
