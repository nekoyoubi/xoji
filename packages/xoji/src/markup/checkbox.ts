...
const GROUP_INDICATOR =
	`<span class="xoji-checkbox__indicator" part="indicator" aria-hidden="true">` +
	`<svg viewBox="0 0 16 16" width="16" height="16">` +
	`<path class="xoji-checkbox__check" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" d="m4 8 3 3 5-6" />` +
	`<path class="xoji-checkbox__dash" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" d="M4 8h8" />` +
	`</svg></span>`;
...
export function checkboxGroupHeadingClass(props: CheckboxGroupMarkupProps): string {
	...
	return [
		"xoji-checkbox",
		"xoji-checkbox-group__heading",
		`xoji-checkbox--${tone}`,
		size !== "md" && `xoji-checkbox--${size}`,
		props.indeterminate && "xoji-checkbox--indeterminate",
	]
		.filter(Boolean)
		.join(" ");
}
 ...
 * renders it at runtime; the `@xoji/astro` binding emits the same string into a declarative
...
export function checkboxGroupMarkup(props: CheckboxGroupMarkupProps): string {
	...
	const heading =
		...
		`<span class="xoji-checkbox__box" part="box">` +
		`<input part="control" class="xoji-checkbox__control" type="checkbox"${checkedAttr}${disabledAttr} />` +
		GROUP_INDICATOR +
		`</span>` +
		`<span class="xoji-checkbox__label" part="label" id="${props.labelId}">${label}</span>` +
		`</label>`;
	let nameAttr = "";
	...
	return `<div role="group" class="xoji-checkbox-group"${nameAttr}${ariaDisabled}>${heading}<div class="xoji-checkbox-group__items" part="items"><slot></slot></div></div>`;
}
