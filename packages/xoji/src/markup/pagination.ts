...
function pageItem(n: number, current: number, href: string | undefined): string {
	...
	const cell =
		href !== undefined
			? `<a class="xoji-pagination__page" part="page" href="${escapeAttr(paginationHref(href, n))}" aria-label="${escapeAttr(label)}" data-page="${n}">${n}</a>`
 ...
 * its shadow root at runtime; the `@xoji/astro` binding emits the same string into a declarative
