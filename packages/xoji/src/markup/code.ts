...
export function codeMarkup(props: CodeMarkupProps): string {
	...
	return (
		`<pre part="pre" class="xoji-code ${cls}"><code part="code" class="${cls}">${props.html}</code></pre>` +
		`<button part="copy" class="xoji-code-copy" type="button" data-copy aria-label="Copy code" hidden>` +
		`<span class="xoji-code-copy-label" data-copy-label>Copy</span></button>`
	);
}
 ...
 * `.xoji-code-line` row so a CSS counter can number it and a `highlight` set can tint
...
export function splitCodeLines(html: string, highlight?: ReadonlySet<number>): { html: string; lines: number } {
	...
	return {
		html: rows
			.map((r, i) => `<span class="xoji-code-line"${highlight?.has(i + 1) ? " data-line-highlight" : ""}><span class="xoji-code-line__text">${r || "\u200b"}</span></span>`)
			.join(""),
		lines: rows.length,
	};
}
