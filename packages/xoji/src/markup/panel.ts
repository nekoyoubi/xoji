...
export function panelClass(props: PanelMarkupProps): string {
	...
	return [
		"xoji-panel",
		variant !== "default" && `xoji-panel--${variant}`,
		props.scrollable && "xoji-panel--scroll",
	]
		.filter(Boolean)
		.join(" ");
}
...
function marker(): string {
	return `<span class="xoji-panel__marker" part="marker" aria-hidden="true"><svg viewBox="0 0 24 24" width="1em" height="1em"><path fill="currentColor" d="M9 6l6 6-6 6V6Z" /></svg></span>`;
}

function titleHeading(props: PanelMarkupProps): string {
	const tag = `h${level(props)}`;
	return `<${tag} class="xoji-panel__title" part="title" id="${props.titleId}">${props.title ?? ""}</${tag}>`;
}
...
export function panelMarkup(props: PanelMarkupProps): string {
	if ((props.variant ?? "default") === "collapsible") {
		...
		return `<section class="${panelClass(props)}" part="panel">
				<button class="xoji-panel__toggle" part="header" type="button" aria-expanded="${expanded}" aria-controls="${props.titleId}-region">${marker()}<span class="xoji-panel__title" part="title" id="${props.titleId}">${props.title ?? ""}</span><span class="xoji-panel__spacer" part="spacer"></span><slot name="actions"></slot></button>
				<div class="xoji-panel__collapse" id="${props.titleId}-region" role="region" aria-labelledby="${props.titleId}"${props.open ? "" : " hidden"}>${body(props)}</div>
			</section>`;
	}
	const header = hasHeader(props)
		? `<header class="xoji-panel__header" part="header">${props.title ? titleHeading(props) : ""}<span class="xoji-panel__spacer" part="spacer"></span><slot name="actions"></slot></header>`
		: "";
