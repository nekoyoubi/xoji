...
export function selectClass(props: SelectMarkupProps): string {
	...
	return [
		"xoji-select",
		size === "sm" && "xoji-select--sm",
		size === "lg" && "xoji-select--lg",
		props.invalid && "xoji-select--invalid",
	]
		.filter(Boolean)
		.join(" ");
}
...
export function selectMarkup(props: SelectMarkupProps): string {
	...
	const selectAttrs = [
		`class="xoji-control xoji-select__field"`,
		`part="select"`,
	...
	return `
				<div class="${selectClass(props)}" part="root">
					<label class="xoji-select__label" part="label" for="${escapeAttr(props.fieldId)}"${labelHidden}>${escape(label)}</label>
					<div class="xoji-select__control" part="control">
						<select ${selectAttrs}>${props.optionsHtml ?? ""}</select>
						<span class="xoji-select__chevron" part="chevron" aria-hidden="true">
					...
					<span class="xoji-select__error" part="error" id="${escapeAttr(props.errorId)}"${errorHidden}>${escape(error)}</span>
				</div>
			`;
}
