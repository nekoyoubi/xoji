...
export function toastIconMarkup(severity: string): string {
	...
	return path
		? `<span class="xoji-toast__icon" part="icon" aria-hidden="true"><svg viewBox="0 0 24 24" width="1em" height="1em">${path}</svg></span>`
		: "";
}
...
export function toastClass(props: ToastMarkupProps): string {
	...
	return [
		"xoji-toast",
		`xoji-toast--${variant}`,
		`xoji-toast--${color || "info"}`,
		severity ? "" : "xoji-toast--noicon",
	]
		.filter(Boolean)
		.join(" ");
}
...
export function toastMarkup(props: ToastMarkupProps): string {
	...
	const action = actionLabel
		? `<button class="xoji-toast__action" part="action" type="button">${actionLabel}</button>`
		: "";
	const closable = props.closable ?? true;
	const close = closable
		? `<button class="xoji-toast__close" part="close" type="button" aria-label="${props.closeLabel ?? "Dismiss"}">${CLOSE_ICON}</button>`
		: "";
	return `<div class="${toastClass(props)}" part="toast" role="${role}" aria-live="${live}" aria-atomic="true">${toastIconMarkup(severity)}<div class="xoji-toast__body" part="body"><span class="xoji-toast__message" part="message"><slot></slot></span>${action}</div>${close}</div>`;
}
...
export function toastRegionClass(props: ToastRegionMarkupProps): string {
	...
	return ["xoji-toast-region", placement !== "bottom-right" && `xoji-toast-region--${placement}`]
		.filter(Boolean)
		.join(" ");
}
