...
// real-world brand colors. This pins a handful of them so a regression that breaks
// AA on, say, a hot-pink brand or a navy-on-light corporate theme fails loudly
// instead of waiting for a random draw to happen to land nearby.
...
describe("real-world brand palettes derive AA-clean", () => {
	for (const { name, constraints } of PALETTES) {
		const register = derive(xojiDefault, { constraints });
