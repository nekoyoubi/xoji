...
import { nxiNite, xojiDefault } from "../src/batteries.js";
...
describe("single-pass backward compat", () => {
	it("a shipped algorithm exposes exactly one pass named 'settle'", () => {
		expect(xojiDefault.passes?.map((p) => p.name)).toEqual(["settle"]);
	});
	...
	it("resolveGraph(lineage) reproduces derive for a single-pass algorithm", () => {
		...
		expect(JSON.stringify(resolveGraph(xojiDefault.lineage(opts) as Parameters<typeof resolveGraph>[0]))).toBe(
			JSON.stringify(xojiDefault.derive(opts)),
		);
	});
});
...
describe("multi-pass algorithm via makeXojiPipelineAlgorithm", () => {
	const multi = makeXojiPipelineAlgorithm(PRESET, (preset, opts) => [
	...
	it("its settle-only sibling is byte-identical to the shipped single-pass form", () => {
		const sibling = makeXojiPipelineAlgorithm(PRESET, (preset, opts) => [settlePass(preset, opts)]);
		const plain = makeXojiAlgorithm(PRESET);
