...
const ALGORITHM_IDS = ["xoji-default", "xoji-hc", "xoji-quiet", "xoji-loud", "nxi-nite"] as const;
...
const DEPTH = process.env.XOJI_GAUNTLET_DEPTH;
...
if (!full) {
	console.info(
		`mod-host: byte-identical proof sampled to ${matrix.length}/${fullMatrix.length} cases ` +
			`(set XOJI_GAUNTLET_DEPTH=full for the whole matrix)`,
	);
}
