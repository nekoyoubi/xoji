...
describe("theme file", () => {
	it("builds a self-describing envelope with format markers, meta, recipe, and tokens", () => {
		...
		expect(file.recipe.algorithm).toBe("xoji-default");
	...
	it("round-trips through serialize and parse", () => {
		...
		expect(parsed?.recipe.algorithm).toBe("xoji-default");
	...
	it("re-derives the cached tokens byte-identically from the recipe", () => {
		...
		const cached = derive(xojiDefault, { constraints: pinnedRecipe.overrides });
		const parsed = parseThemeFile(
			serializeThemeFile(buildThemeFile({ meta: { name: "Cache" }, recipe: pinnedRecipe, register: cached })),
		);
		const rederived = derive(xojiDefault, { constraints: parsed?.recipe.overrides });
	...
	it("rejects non-envelope JSON and junk", () => {
		...
		expect(parseThemeFile(JSON.stringify({ format: "xoji-theme" }))).toBeNull();
		expect(isThemeFile({ format: "xoji-theme", recipe: { algorithm: "x" } })).toBe(true);
