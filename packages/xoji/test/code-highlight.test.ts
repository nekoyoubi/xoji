...
describe("registerLanguage", () => {
	it("makes a custom grammar resolvable and used", async () => {
		registerLanguage("xoji-toy", {
			keyword: /\b(?:THEME|TOKEN)\b/,
		});
		expect(resolveLanguage("xoji-toy")).toBe("xoji-toy");
		const result = await highlight("THEME and TOKEN", "xoji-toy");
		expect(result.language).toBe("xoji-toy");
		expect(result.html).toContain('class="token keyword"');
