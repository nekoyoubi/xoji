...
1. **The neutral importable API still leads with baked.** `@xoji/core`'s environment-neutral entry exposes `getAlgorithm` (baked) as the importable path; `resolveAlgorithm` is the Node-side host seam and hasn't been generalized to the browser/importable surface. Baked stays the byte-identical oracle regardless.
